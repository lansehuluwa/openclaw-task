...
Agent 可通过 `agents[].model` 的 `provider/model` 写法选择服务；同名
`simulator_config` 配置优先。服务地址和明文密钥写在 `models.json`。详见
...
- **[Pi RPC 集成说明](docs/PI_RPC_INTEGRATION_ASSESSMENT.md)** - RPC、provider、workspace、skill、session 与测试
...
| **Pi** | 单轮问答 | 多轮 + simulator | 多轮 + simulator + evaluator |
