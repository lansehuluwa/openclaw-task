...
class CodexClientLifecycleTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_same_session_reuses_thread_and_other_session_isolated(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            agent_workspace = Path(temp_dir) / "agent"
            agent_workspace.mkdir()
            (agent_workspace / "INPUT.md").write_text("input", encoding="utf-8")
            ...
            self.assertEqual(
                sdk.thread_start_calls[0]["cwd"],
                str(agent_workspace / ".sessions" / "main"),
            )
            self.assertEqual(
                sdk.thread_start_calls[1]["cwd"],
                str(agent_workspace / ".sessions" / "other"),
            )
            self.assertTrue(
                (agent_workspace / ".sessions" / "main" / "INPUT.md").is_file()
            )
...
class CodexGenericConfigTests(unittest.TestCase):
    def test_three_shared_configs_use_fixed_codex_workspace(self):
        config_names = (
            "config_simple.json",
            "config_user.json",
            "config_simple_eval.json",
        )
        with patch("src.codex_client.CodexWorkspaceManager") as manager_class:
            ...
            for config_name in config_names:
                with self.subTest(config=config_name):
                    ...
                    manager_class.assert_called_once_with("~/.codex/workspace")
